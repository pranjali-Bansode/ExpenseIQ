...
_tmp_db.close()
os.environ["EXPENSEIQ_TEST_DB"] = _tmp_db.name  # consumed below if db.py honours it
...
def _setup_db():
    ...
    cursor = conn.execute(
        ...
        ("Test User", "test@expenseiq.com", generate_password_hash("password123")),
    )
...
def _login(c, email="test@expenseiq.com", password="password123"):
...
class TestQueryHelpers:
    ...
    def _user_id(self):
        conn = get_db()
        row = conn.execute(
            "SELECT id FROM users WHERE email = ?", ("test@expenseiq.com",)
        ).fetchone()
        conn.close()
        return row["id"]
